...
logging.basicConfig(filename="test.log",level=logging.INFO,
                    format=log_format,datefmt=time_format) #log名字，等级设定，log默认生成在该py文件所在目录
...
def ping():
    ...
    if backinfo == 0: #返回0为True
        ...
        ping_button = tk.Button(window,text="connect",
                                font=("Arial",20),
                                command=ping).grid(row=0,column=2) #设置ping button，放在第一行，第二列
...
start_button = tk.Button(window,text="Start",font=("Arial",20),
                         command=lambda:hit("start")).place(x=100,y=100) #设置开始button
stop_button = tk.Button(window,text="Stop",font=("Arial",20),
                                                command=lambda:hit("stop")).place(x=200,y=100) #设置停止button
