    ...
    for workspace in workspaces:
        ...
        try:
            ...
            for fc in arcpy.ListFeatureClasses():
                ...
                arcpy.Statistics_analysis(in_fc_path, st_table_path, [["Shape_Area", "SUM"]], 'DLBM')

                with arcpy.da.SearchCursor(st_table_path, ('DLBM','FREQUENCY','SUM_Shape_Area')) as cursor1:
                    for row1 in cursor1:
                        if str(row1[0]).strip() not in resultdic:
                            all_fields.append(str(row1[0]).strip())
                        if str(row1[0]).strip() not in resultdic:
                            resultdic.setdefault(str(row1[0]).strip(),row1[2])
                        else:
                            resultdic[str(row1[0]).strip()] += row1[2]
        ...
        if success:
            try:
                ...
                # field_names = [f.name for f in old_fields if f.name != "OBJECTID"]
                ...
                for key in dicaa:
                    # arcpy.AddMessage("{0}, {1}".format(key[0], key[1]))
                ...
                for tfield in new_fields:
                    # if tfield.name != "OBJECTID" and tfield.aliasName in resultdic:
                    if tfield.name != "OBJECTID":
                        field_names.append(tfield.name)
                        # row_values.append(resultdic[str(tfield.aliasName).decode("utf-8")])
                        row_values.append(resultdic.get(str(tfield.aliasName).decode("utf-8"),0))
                        # arcpy.AddMessage("      {0}-{1},  {2}".format(tfield.name, tfield.aliasName, resultdic[str(tfield.aliasName).decode("utf-8")]))
                # arcpy.AddMessage("+" * 15)
