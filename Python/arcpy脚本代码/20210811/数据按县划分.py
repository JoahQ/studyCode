...
def xzqabGetCode(name):
    if "兴宁区" in name:
        ...
        return "450224"
    elif "融水苗族自治县" in name:
        return "450225"
    elif "三江侗族自治县" in name:
        return "450226"
        ...
        return "450327"
    elif "龙胜各族自治县" in name:
        return "450328"
    elif "资源县" in name:
        return "450329"
    elif "平乐县" in name:
        return "450330"
    elif "恭城瑶族自治县" in name:
        return "450332"
    elif "荔浦市" in name:
        return "450381"
        ...
        return "451224"
    elif "罗城仫佬族自治县" in name:
        return "451225"
    elif "环江毛南族自治县" in name:
        return "451226"
    elif "巴马瑶族自治县" in name:
        return "451227"
    elif "都安瑶族自治县" in name:
        return "451228"
    elif "大化瑶族自治县" in name:
        return "451229"
...
def huafenxian(in_path,out_path,gdbname,xzq_feild):
    cout = 0
    fail = 0
    warning = 0
    fail_list = []
    ...
    list_city = []
    with arcpy.da.SearchCursor(in_path, xzq_feild) as cursor:
        for row in cursor:
            if str(row[0]).strip() not in list_city:
                list_city.append(str(row[0]).strip())
                cout += 1
    ...
    for city in list_city:
        out_gdb_name = xzqabGetCode(city) + city + gdbname + u".gdb"
        ...
        expr_sec = xzq_feild + " like \'%" + str(city) + "%\'"
        try:
...
#         out_gdb_name = code + xzqabGetCode(code) + gdbname
#         out_gdb = os.path.join(out_path, out_gdb_name)
#         arcpy.AddMessage("=" * 60)
#         cout += 1
#         arcpy.AddMessage(" (" + str(cout) + ") " + out_gdb_name)
...
if __name__ == "__main__":
    ...
    huafenxian(in_pathg,out_patha,out_name,ffff)