...
def BatchIntersect(in_path, layer_name,consult_workspace, out_path):
    ...
    arcpy.env.workspace = consult_workspace
    consult_fc_list = arcpy.ListFeatureClasses()
    ...
    for workspace in workspaces:
        ...
        try:
            ...
            if arcpy.Exists(infc_path):
                ...
                consult_fc_pathp = ""
                for consult_fc in consult_fc_list:
                    if xzqcode[0] in consult_fc:
                        consult_fc_pathp = os.path.join(consult_workspace, consult_fc)
                        break
                ...
                arcpy.Intersect_analysis([infc_path, consult_fc_pathp], erase1_path)
            else:
...
if __name__ == "__main__":
    ...
    consult_workspaceG = arcpy.GetParameterAsText(2)
    out_pathG = arcpy.GetParameterAsText(3)

    BatchIntersect(in_pathG, layer_nameG,consult_workspaceG, out_pathG)
