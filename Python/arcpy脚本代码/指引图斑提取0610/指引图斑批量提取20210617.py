...
def selectFeatureClass(in_dltb, in_xztj, podu_path1, standard_gdb, out_gdb1):
    ...
    for fci in fclist:
        ...
        if u"建设用地" in fci:
            ...
            del fieldmap2
            del fieldmappings
        arcpy.AddMessage(u"         XDM、XMC、TBMJ赋值，TBLX标注恢复属性...")
...
def aggregateAanalysisQ(in_path2,where_clause2,out_gdb2):
    ...
    fc123 = os.path.basename(in_path2)
    select_1 = os.path.join(out_gdb2,getNewFileNameQ(out_gdb2,  fc123 + u"筛选大于400"))
    buffer_result = os.path.join(out_gdb2, getNewFileNameQ(out_gdb2, fc123 + u"缓冲结果"))
    dissolve_result = os.path.join(out_gdb2, getNewFileNameQ(out_gdb2, fc123 + u"融合结果"))
    spatialJoin = os.path.join(out_gdb2, getNewFileNameQ(out_gdb2, fc123 + u"空间连接"))
    spatialJoin_Statisti = os.path.join(out_gdb2, getNewFileNameQ(out_gdb2, fc123 + u"汇总统计表"))
    tableSelect = os.path.join(out_gdb2, getNewFileNameQ(out_gdb2, fc123 + u"筛选表"))
    last_result = os.path.join(out_gdb2, getNewFileNameQ(out_gdb2, fc123 + u"聚合结果"))
...
if __name__ == "__main__":
    in_path = arcpy.GetParameterAsText(0)#三调地类图斑文件夹
    in_XZTJ_path = arcpy.GetParameterAsText(1)#限制条件文件夹
    in_pdg_path = arcpy.GetParameterAsText(2)#坡度文件夹
    templetg = arcpy.GetParameterAsText(3)#标准库模板
    out_path = arcpy.GetParameterAsText(4)#输出位置

    arcpy.env.workspace = in_XZTJ_path
    xztj_ws = arcpy.ListWorkspaces("*","ALL")

    arcpy.env.workspace = in_pdg_path
    pd_ws = arcpy.ListWorkspaces("*", "ALL")
    ...
    for workspace in workspaces:
        in_gdb_name = os.path.basename(workspace)
        cout += 1
        arcpy.AddMessage("(" + str(cout) + ")" + in_gdb_name)
        temp_name = getNewFileNameQ(out_path, "temp.gdb")
        temp_gdb = os.path.join(out_path, temp_name)
        try:
            xzq_code = re.findall(r"\d+", in_gdb_name)[0]
            asa = re.findall(r"[^()（）。\[\]{},*/|\\:?\'\"<>]", in_gdb_name)
            namegdb = getNewFileNameQ(out_path, u"指引图斑" + "".join(asa))
            DLTB_PATH_1 = os.path.join(workspace, "DLTB")#####注意这里
            in_XZTJg = ""
            in_pdg = ""
            for xztj in xztj_ws:
                if xzq_code in os.path.basename(xztj):
                    arcpy.env.workspace = xztj
                    for fc in arcpy.ListFeatureClasses():
                        in_XZTJg = os.path.join(xztj, fc)
                        arcpy.AddMessage(u"  限制条件：" + in_XZTJg)
                        break
                    break
            for pd in pd_ws:
                if xzq_code in os.path.basename(pd):
                    arcpy.env.workspace = pd
                    for fc in arcpy.ListFeatureClasses():
                        in_pdg = os.path.join(pd, fc)
                        arcpy.AddMessage(u"  坡度图：" + in_pdg)
                        break
                    break

            if arcpy.Exists(in_XZTJg) and arcpy.Exists(in_pdg):
                ...
                # arcpy.AddMessage(DLTB_PATH_1)
                gdb_result = createGDBAndFeatureclass(out_path, namegdb,templetg,DLTB_PATH_1)
                arcpy.AddMessage("   ")
                arcpy.AddMessage(u"  提取图斑")
                selectFeatureClass(DLTB_PATH_1,in_XZTJg,in_pdg,gdb_result,temp_gdb)
                arcpy.AddMessage(u"  完成提取！")
                arcpy.AddMessage(u"  ")
            elif not arcpy.Exists(in_XZTJg):
                arcpy.AddError(u"    没有找到与 " + in_gdb_name + u" 对应的限制条件。请检查输入参数！")
                fail += 1
                fail_list.append(in_gdb_name)
                continue
            elif not arcpy.Exists(in_pdg):
                arcpy.AddError(u"    没有找到与 " + in_gdb_name + u" 对应的坡度图。请检查输入参数！")
                fail += 1
                fail_list.append(in_gdb_name)
                continue
    ...
    arcpy.AddMessage(" ")
    arcpy.AddMessage('+'*60)
    arcpy.AddMessage(u"  成功：" + str(cout - fail) + u" 个！")
    if fail > 0:
