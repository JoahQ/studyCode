...
def statisticsArea1(in_path, out_path, statistics_fields, case_fields,lastname):
    cout = 0
    fail = 0
    fail_list = []
    ...
    try:
        ...
        lastMergemane = getNewFileNameQ(out_path, lastname)
        lastMergemane_gdb = os.path.join(out_path, lastMergemane)
        arcpy.AddMessage("新建 " + result_gdb + " 用于保存结果属性表和过程数据...")
        arcpy.CreateFileGDB_management(out_path, lastMergemane)
        arcpy.Merge_management(table_lists, table_lists_merge)
    except Exception as e:
        arcpy.AddError(e.message)
...
if __name__ == "__main__":
    ...
    case_fieldsG = arcpy.GetParameterAsText(2)

    statisticsArea1(in_pathG, out_pathG, [["Shape_Area", "SUM"]], case_fieldsG)
