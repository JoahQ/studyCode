    ...
    for workspace in workspaces:
        ...
        try:
            for iin_fc_path in right_name_path[1]:
                ...
                arcpy.Statistics_analysis(merge_fc_path, nyd_tb1, [["Shape_Area", "SUM"]], ['TBLX','GDHBZY'])
                row_valuen = []
                row_count = []
                with arcpy.da.SearchCursor(nyd_tb1, ('TBLX')) as cursorx:
                    for rowx in cursorx:
                        rlist = "".join(re.findall(r"[^\d+.% ]", str(rowx))).split("/")
                        for rr in rlist:
                            if rr.strip() not in tblx_list:
                                tblx_list.append(rr)
                                row_valuen.append(0)
                                row_count.append(0)
                                # tblx_cout += 1
                ...
                with arcpy.da.SearchCursor(nyd_tb1, ('GDHBZY', 'TBLX','FREQUENCY', 'SUM_Shape_Area')) as cursor1:
                    for row1 in cursor1:
                        for rr2 in tblx_list:
                            if str(rr2) in str(row1[0]):
                                row_valuen[rr2] += row1[2]
                                row_count[rr2] += row1[1]

                for rr2 in range(len(row_valuen)):
                    row_values.append(row_valuen[rr2])
                    row_values.append(row_count[rr2])
