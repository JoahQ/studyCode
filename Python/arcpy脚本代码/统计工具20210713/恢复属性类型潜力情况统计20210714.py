...
def createTableQ(path, name):
    ...
    field_list = [{'name': 'cout', 'type': 'TEXT', 'alias': '序号'},  #0
                  ...
                  {'name': 'C2', 'type': 'DOUBLE', 'alias': '宜耕农用地潜力-工程恢复总'},  #3
                  {'name': 'G3', 'type': 'DOUBLE', 'alias': '工程恢复-02'},  # 3
                  {'name': 'O4', 'type': 'DOUBLE', 'alias': '工程恢复-03'},  # 11
                  {'name': 'V5', 'type': 'DOUBLE', 'alias': '工程恢复-04'},  # 18
                  {'name': 'X6', 'type': 'DOUBLE', 'alias': '工程恢复-11'},  # 20

                  {'name': 'F7', 'type': 'DOUBLE', 'alias': '宜耕农用地潜力-即可恢复总'},  # 2
                  {'name': 'G8', 'type': 'DOUBLE', 'alias': '即可恢复-02'},  # 3
                  {'name': 'O9', 'type': 'DOUBLE', 'alias': '即可恢复-03'},  # 11
                  {'name': 'V10', 'type': 'DOUBLE', 'alias': '即可恢复-04'},  # 18
                  {'name': 'X11', 'type': 'DOUBLE', 'alias': '即可恢复-11'},  # 20

                  {'name': 'C12', 'type': 'DOUBLE', 'alias': '提质改造潜力-工程恢复总'},  # 3
                  {'name': 'G13', 'type': 'DOUBLE', 'alias': '工程恢复-02'},  # 3
                  {'name': 'O14', 'type': 'DOUBLE', 'alias': '工程恢复-03'},  # 11
                  {'name': 'V15', 'type': 'DOUBLE', 'alias': '工程恢复-04'},  # 18
                  {'name': 'X16', 'type': 'DOUBLE', 'alias': '工程恢复-11'},  # 20

                  {'name': 'F17', 'type': 'DOUBLE', 'alias': '提质改造潜力-即可恢复总'},  # 2
                  {'name': 'G18', 'type': 'DOUBLE', 'alias': '即可恢复-02'},  # 3
                  {'name': 'O19', 'type': 'DOUBLE', 'alias': '即可恢复-03'},  # 11
                  {'name': 'V20', 'type': 'DOUBLE', 'alias': '即可恢复-04'},  # 18
                  {'name': 'X21', 'type': 'DOUBLE', 'alias': '即可恢复-11'},  # 20
                  ]
...
def statisticsAreaQ3(in_path, out_path, where_clause, name):
    ...
    tb_paths = createTableQ(result_gdb, getNewFileNameQ(result_gdb, "a" + name))
    ...
    for workspace in workspaces:
        ...
        try:
            ...
            merge_fc_path = os.path.join(merge_gdb, "合并" + xzqcode[0]+xzqab(xzqcode[0]))
            merge_table_path = os.path.join(result_gdb, "汇总统计" + xzqcode[0]+xzqab(xzqcode[0]))
            ...
            while i < len(field_ls):
                row_values.append(0)
                i += 1
            out_featuren = os.path.join(result_gdb, getNewFileNameQ(result_gdb, "PJJG是" + xzqcode[0]+xzqab(xzqcode[0])))
            arcpy.Select_analysis(merge_fc_path, out_featuren, where_clause + u" like \'%是%\'")

            arcpy.Statistics_analysis(out_featuren, merge_table_path, [["Shape_Area", "SUM"]], ['DLBM', 'ZZSXMC','GDHBZY'])


            with arcpy.da.SearchCursor(merge_table_path, ('DLBM','ZZSXMC','GDHBZY', 'SUM_Shape_Area')) as cursor1:
                for row1 in cursor1:
                    if "农用地" in str(row1[2]):
...
def tableToExcelQ(excel_path3, datalist, sheets_name3='sheet'):
    ...
    def add_new_sheet(sheet_name):
        ...
        worksheets.write_merge(1, 1, 0, 21, u'面积：平方米', style2)

        worksheets.write_merge(2, 3, 0, 0, u'序号', style1)
        worksheets.write_merge(2, 3, 1, 1, u'行政区名称', style1)

        worksheets.write_merge(2, 2, 2, 11, u'宜耕农用地潜力调查图斑', style1)
        worksheets.write_merge(2, 2, 12, 21, u'耕地提质改造潜力调查图斑', style1)

        worksheets.write(3, 2, u'工程恢复总面积', style1)
        worksheets.write(3, 3, u'工程恢复-02', style1)
        worksheets.write(3, 4, u'工程恢复-03', style1)
        worksheets.write(3, 5, u'工程恢复-04', style1)
        worksheets.write(3, 6, u'工程恢复-11', style1)

        worksheets.write(3, 7, u'即可恢复总面积', style1)
        worksheets.write(3, 8, u'即可恢复-02', style1)
        worksheets.write(3, 9, u'即可恢复-03', style1)
        worksheets.write(3, 10, u'即可恢复-04', style1)
        worksheets.write(3, 11, u'即可恢复-11', style1)


        worksheets.write(3, 12, u'工程恢复总面积', style1)
        worksheets.write(3, 13, u'工程恢复-02', style1)
        worksheets.write(3, 14, u'工程恢复-03', style1)
        worksheets.write(3, 15, u'工程恢复-04', style1)
        worksheets.write(3, 16, u'工程恢复-11', style1)

        worksheets.write(3, 17, u'即可恢复总面积', style1)
        worksheets.write(3, 18, u'即可恢复-02', style1)
        worksheets.write(3, 19, u'即可恢复-03', style1)
        worksheets.write(3, 20, u'即可恢复-04', style1)
        worksheets.write(3, 21, u'即可恢复-11', style1)
