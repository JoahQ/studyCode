...
def findAndCopyFC(in_path,out_path,addname,in_fc_name,sql_clause):
    cout = 0
    fail = 0
    warning = 0
    fail_list = []
    ...
    for workspace in workspaces:
        ...
        try:
            ...
            if arcpy.Exists(in_fc_path1):
                arcpy.AddMessage(in_fc_path1 + u" 筛选图斑...")
                out_fc_p = os.path.join(out_gdb_path,in_fc_name)
                arcpy.Select_analysis(in_fc_path1, out_fc_p, sql_clause)
                continue
            ...
            for dataset in dataset_ls:
                ...
                if arcpy.Exists(in_fc_path2):
                    arcpy.AddMessage(in_fc_path2 + u" 筛选图斑...")
                    out_fc_p2 = os.path.join(out_gdb_path, in_fc_name)
                    arcpy.Select_analysis(in_fc_path1, out_fc_p2, sql_clause)
                    break
                else:
                    exist_fc = False

            if not exist_fc:
                arcpy.AddError("    " + os.path.basename(workspace) + " 中没有 " + in_fc_name + " 图层！")
...
if __name__ == "__main__":
    ...
    sql_clauseg = arcpy.GetParameterAsText(4)

    findAndCopyFC(in_pathg,out_pathg,add_nameg,fc_nameg,sql_clauseg)
